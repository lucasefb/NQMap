export default {
  async loadKMZ() {
    this.isLoading = true;
    try {
      await this.processKMZFiles(this.filterForRSRP);
    } catch (error) {
  ...
  async processKMZFiles(filter) {
    for (const [key, value] of Object.entries(filter)) {
      if (value === true && !this.loadedKMZFiles.has(key)) {
        ...
        try {
          const response = await this.$axios.get(`http://localhost:3000/api/get-kmz/${filename}`);
          ...
          for (const overlay of groundoverlays) {
            ...
            href = href.replace(/\\/g, '/');
            const imageUrl = `http://localhost:3000${href}`;
  ...
  updateKMZLayer() {
    const allFalse = Object.values(this.filterForRSRP).every(v => v === false);
    ...
    const activeKeys = Object.entries(this.filterForRSRP)
